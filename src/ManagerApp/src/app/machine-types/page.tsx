...
export default function MachineTypesPage() {
  ...
  const loadMachineTypes = async () => {
    try {
      ...
      setError('Không thể tải danh sách loại máy');
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Bạn có chắc chắn muốn xóa machine type này?')) {
      try {
        ...
        setError('Không thể xóa loại máy');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(editingMachineType ? 'Không thể cập nhật loại máy' : 'Không thể tạo loại máy mới');
      console.error('Error saving machine type:', err);
    }
  };

  const MachineTypeModal = () => (
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Quản lý Machine Types</h1>
            <p className="text-gray-600">Quản lý các loại máy móc</p>
          </div>
          <button
            onClick={handleAdd}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tên Machine Type</th>
